...
class OssAdapter extends AbstractAdapter
{
    ...
    public function writeStream($path, $resource, Config $config)
    {
        ...
        while (!feof($resource)) {
            ...
            $this->oss->appendObject($this->bucket, $path, $buffer, $position, $this->getOssOptions($config));
            ++$i;
        }
        fclose($resource);
    ...
    public function ossImageInfo($path, $modify = 'image/info', $params = null)
    {
        if ('/' === $path[0]) {
            $path = substr($path, 1);
        }

        return json_decode($this->oss->getObject($this->bucket, $path, [
            OssClient::OSS_PROCESS => $modify.$params ?: '',
        ]), true);
    }
